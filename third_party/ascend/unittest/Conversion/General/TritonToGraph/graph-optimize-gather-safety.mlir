// RUN: triton-opt %s --split-input-file --verify-each -graph-optimize='target-arch=Ascend910B1 rule-mask=512 ub-capacity-bytes=98304 compile-mode=simd' | FileCheck %s --implicit-check-not=arith.shrsi
// RUN: triton-opt %s --split-input-file --verify-each -graph-optimize='target-arch=Ascend910B1 rule-mask=512 ub-capacity-bytes=371 compile-mode=simd' | FileCheck %s
// RUN: triton-opt %s --split-input-file --verify-each -graph-optimize='target-arch=Ascend910B1 rule-mask=512 ub-capacity-bytes=370 compile-mode=simd' | FileCheck %s --check-prefix=SMALL --implicit-check-not=tt.gather
