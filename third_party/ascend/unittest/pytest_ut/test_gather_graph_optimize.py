...
def make_indirect_ttir(rule_mask, *, per_lane=False, volatile=False, index_type="i32", value_type="fp32",
    ...
    options = NPUOptions(arch=arch, graph_optimize_rule_mask=rule_mask, **mode_options)
    source = ASTSource(
...
def test_gather_rule_target_and_mode(rule_mask, arch, compile_mode):
    ...
    expected = bool(rule_mask & 512) and arch in ("Ascend910B1", "Ascend910_9391")
    assert ("tt.gather" in text) == expected
...
@pytest.mark.parametrize("case", [
    "tail", "negative", "negative_below_bound", "negative_upper_bound", "upper_bound", "all_masked", "per_lane",
    "volatile", "i64",
])
...
def test_gather_rule_npu_full_row_offset_view(monkeypatch, compile_mode):
    ...
    for rule_mask in (511, 1023):
        ...
        compiled = indirect_rows_kernel[(1, )](source, indices, output, 2, WIDTH=16, K=8, ROW_BLK=2, ROW_STEP=2,
                                               PER_LANE=False, VOLATILE=False, BASE_ROW=0,
                                               graph_optimize_rule_mask=rule_mask, **mode_options)
        torch.npu.synchronize()
        expected_rewrite = gather_target and rule_mask == 1023
        assert ("tt.gather" in compiled.asm["ttir"]) == expected_rewrite
...
def test_gather_rule_npu_negative_debug(tmp_path, monkeypatch):
    ...
    try:
        ...
        if not target.arch.startswith(("Ascend910B", "Ascend910_93")):
            log("SKIP: this target does not enable automatic Gather in v3")
        ...
        log("511=raw-load baseline; 512=Gather rule selected; 1023=default rules including Gather")
        ...
        for rule_mask in masks:
            ...
            compiled = indirect_rows_kernel.warmup(source_npu, indices_npu, output_npu, n_rows, grid=(1, ),
                                                   graph_optimize_rule_mask=rule_mask, **options)
        ...
        for case, indices in cases.items():
            ...
            log(f"expected mask=511 (raw offsets)={raw.tolist()}")
            log(f"expected masks=512/1023 (tile-wise wrap/fallback)={expected.tolist()}")
            ...
            for rule_mask in masks:
                ...
                log(f"START {stage}")
                launched = indirect_rows_kernel[(1, )](source_npu, indices_npu, output_npu, n_rows,
                                                       graph_optimize_rule_mask=rule_mask, **options)
                ...
                reference = raw if rule_mask == 511 else expected
